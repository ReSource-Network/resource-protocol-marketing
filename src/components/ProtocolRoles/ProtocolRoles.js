...
const cardsContent = [operator, merchants, underwriters]
...
class ProtocolRoles extends React.Component {
  ...
  renderCards() {
    return cardsContent.map((card) => (
      <div className={"protocolRolesCard"} key={card.titleText}>
        <img
          className={"protocolRolesCardImg"}
          src={getCloudinaryImagePath(card.imgSrcName)}
          alt={card.imgAltText}
        />
        <h6>{card.titleText}</h6>
      ...
      <section id={"protocolRoles"}>
        {isIOS && isSafari ? null : (
          <video
            ...
            poster={
              "https://res.cloudinary.com/resource-network/image/upload/v1632976534/resource-protocol/animation-poster_yvjxuo.png"
            }
            autoPlay
            muted
            loop
            ...
            preload={"true"}
          >
            <source src={getCloudinaryVideoPath(animationMpVideoSrcName)} type={"video/mp4"} />
            <source src={getCloudinaryVideoPath(animationWebmVideoSrcName)} type={"video/webm"} />
            Your browser does not support the video tag.
          </video>
        )}

        <div id={"protocolRolesCards"}>{this.renderCards()}</div>
      </section>
    )
  }
}
